...
export class Shader {
  ...
  private hsv: HSV = { hue: 0, saturation: 1, brightness: 1 }
  private _mouseMode: MouseMode = 'click'
  private _mouseSensitivity = 1.0 // New: mouse sensitivity multiplier
  private _mouseDamping = 0.9 // New: mouse movement damping factor (0-1)

  private _speed = 1 // Speed multiplier
  ...
  constructor(private container: HTMLElement, mouseMode?: MouseMode, fps?: number) {
  ...
  private setupMouseEvents(): void {
    ...
    canvas.addEventListener('mousemove', (event: MouseEvent) => {
      ...
      this.iMouse.x
        = this.iMouse.x * this._mouseDamping
          + newX * (1 - this._mouseDamping)
      this.iMouse.y
        = this.iMouse.y * this._mouseDamping
          + newY * (1 - this._mouseDamping)
  ...
  private compileProgram(): boolean {
    ...
    const fullFragmentShader
      = this.fragmentShaderHeader + this.shaderSource
  ...
  public setHSV(hsv: Partial<HSV>): void {
